...
// 2.2. 定义Model(与集合UserModel(为函数，new的实例)对应, 可以操作集合增删改查)
const UserModel = mongoose.model('users', userSchema)   //集合名称users  userSchema约束对象

/*3. 通过Model或其实例对集合数据进行CRUD操作*/
// 3.1. 通过Model实例的save()添加数据到数据库
function testSave() {
  const user = {username: 'Jack', password: md5('234'), type: 'dashen'}
  const userModel = new UserModel(user)  //user数据最终保存在集合里
  userModel.save(function (error, userDoc) {  //userDoc为文档
...
function testFind() {
  ...
  UserModel.find({_id:'5b70f19a0ee12107cc440856'}, function (error, usersDoc) { //为文档对象
...
// 3.3. 通过Model的findByIdAndUpdate()更新某个数据  根据id找到user
function testUpdate() {
  const user = {username: 'xxx'}
  UserModel.findByIdAndUpdate({_id:'5b70f19a0ee12107cc440855'}, user, function (error, oldUserDoc) {//更新前的user
