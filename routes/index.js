...
router.post('/register', function (req, res) {
  ...
  UserModel.findOne({username}, (error, userDoc) => {//所有没有名字的函数都可以写成箭头函数  userDoc文档对象
    if(userDoc) {// 此用户已存在
      // 3. 返回响应(失败)
      res.send({code: 1, msg: '此用户已存在, 请重新注册'})
    } else {// 此用户不存在 , 可以注册

      //注册相当于保存信息，就是创建实例，数据保存在集合中                                                           //userDoc生成了新的user
      new UserModel({username, password: md5(password), type}).save((error, userDoc) => {
        ...
        res.json({code: 0, data: {_id: userDoc._id, username, type}})//密码不应该返回_id: userDoc._id
      })
    }
  })
})
...
router.post('/login', function (req, res) {
  ...
  UserModel.findOne({username, password: md5(password)}, filter, (error, userDoc) => {
    if(userDoc) { // 成功                           //filter 为 userDoc里面有密码，要剔除
      // 向浏览器返回一个cookie数据(实现7天免登陆)
      res.cookie('userid', userDoc._id, {maxAge: 1000*60*60*24*7})
     //向浏览器传的为用户的id ，值（user在哪，为user文档中）
